 ...
 * upwards into space, so an inner layer meets an outer layer at TOP -> BOTTOM.
 */
enum class VerticalDimensionFace {
    BOTTOM,
    TOP,
}

data class VerticalDimensionEndpoint(
    val dimension: DimensionId,
    val face: VerticalDimensionFace,
)
...
data class LocalEuclideanPortal(
    val id: String,
    val source: VerticalDimensionEndpoint,
    val target: VerticalDimensionEndpoint,
    val boundary: DimensionBoundaryType,
...
class LocalEuclideanPortalGraph(portals: Collection<LocalEuclideanPortal>) {
    ...
    fun portalAt(dimension: DimensionId, face: VerticalDimensionFace): LocalEuclideanPortal? =
        bySource[VerticalDimensionEndpoint(dimension, face)]

    fun portalsFrom(dimension: DimensionId): List<LocalEuclideanPortal> =
        VerticalDimensionFace.entries.mapNotNull { portalAt(dimension, it) }
}
